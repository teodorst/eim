...
public class GoogleSearcherActivity extends AppCompatActivity {
    ...
    private class SearchGoogleButtonClickListener implements Button.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            String seachKey = searchGoogleButton.getText().toString();
            if (seachKey.isEmpty()) {
                Toast.makeText(getApplicationContext(), Constants.EMPTY_KEYWORD_ERROR_MESSAGE,
                        Toast.LENGTH_LONG).show();
            }
            String[] words = seachKey.split(" ");
            String query = Constants.SEARCH_PREFIX;
            for (String word : words) {
                query += word + "+";
            }
            query = query.substring(0, query.length()-1);
            new GoogleSearcherAsyncTask(googleResultsWebView).execute(query);
        }
    }
